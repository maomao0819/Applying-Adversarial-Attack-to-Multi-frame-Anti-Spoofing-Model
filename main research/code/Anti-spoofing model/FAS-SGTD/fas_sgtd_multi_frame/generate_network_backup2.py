...
def get_logtis_cla_from_logits_list_bk(logits_list):
    ...
    if flags.paras.inverse:
        ...
        logits_cla = tf.concat([1 - logits_cla, logits_cla], axis = 1)
    return logits_cla
...
def build_loss(logits_list, maps, masks, label):
    ...
    loss_reg=tf.reduce_sum(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES))
    # total loss
    ...
    acc = tf.compat.v1.metrics.accuracy(labels=tf.cast(tf.argmax(label,axis=-1),dtype=tf.float32),
                             predictions=tf.cast(tf.argmax(logits,axis=-1),dtype=tf.float32) )
...
def generate_network(features=[], mode=tf.estimator.ModeKeys.TRAIN):
        ...
        for i in range(len_gpus):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('%s_%d' % ("tower", i)) as scope:
                    ...
                    #print("***********************************")
                    #print(logits_list)
                    #print(maps_split[i].shape)
                    #print(masks_split[i].shape)
                    #print(label_split[i].shape)
    ...
    #                     {v.name.split(':')[0]: v for v in variables_to_restore})
    tf.compat.v1.train.init_from_checkpoint(os.path.join( \
                        flags.path.single_frame_model, 'model.ckpt-20000'), 
                        {v.name.split(':')[0]: v for v in variables_to_restore})
