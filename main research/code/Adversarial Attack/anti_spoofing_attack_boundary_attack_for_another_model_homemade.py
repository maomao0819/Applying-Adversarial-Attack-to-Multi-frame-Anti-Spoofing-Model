...
def boundary_attack():
    ...
    model_path = os.path.join('anti_spoofing_optimize', 'REPLAY_homemade_776_new2.h5')
    if model_path:
    ...
    path_save = os.path.join('anti_spoofing_optimize', "adversarial_example_boundary_attack_with_PSNR2/")
    # path_read_attack_image = "anti-spoofing test attack image/"
    path_read_attack_image = os.path.join('anti_spoofing_optimize', 'homemade_attack_choose_cropped_clients')
    # path_read_real_image = "anti-spoofing test real image/"
    path_read_real_image = os.path.join('anti_spoofing_optimize', 'real_image_cropped')
    ...
    for dname in os.listdir(path_read_attack_image):
        if os.path.isdir(os.path.join(path_read_attack_image, dname)):
            ...
            #for img_name in os.listdir(os.path.join(path_read_attack_image, dname)):
                ...
                is_strange_attack_attack_class = False
                ...
                attack_class = (model.predict(initial_sample) > 0.5)[0][0].astype("int32")
                target_class = (model.predict(target_sample) > 0.5)[0][0].astype("int32")
                adversarial_sample = initial_sample
                # real: 1
                # attack: 0
                if attack_class == 0:
                    is_strange_attack_attack_class = True
                    strange_attack_attack_class += 1
                    attack_class = 1
                ...
                while True:
                    ...
                    if (prediction > 0.5)[0][0].astype("int32") == attack_class:
                        ...
                        break
                    elif is_strange_attack_attack_class:
                        epsilon *= 1.1
                    else:
                        epsilon *= 0.9
                while True:
                    ...
                    if n_steps > step_limit:
                        draw(np.copy(moveaxis_change_channel_order(adversarial_sample)), model, os.path.join(path_save, dname, 'real_' + random_image.replace(".png", "") + '_attack_' + img_name))
                        break
                ...
                print("strange_attack_attack_class: ", strange_attack_attack_class)
