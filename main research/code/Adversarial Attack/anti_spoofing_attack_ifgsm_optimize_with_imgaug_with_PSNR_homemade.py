...
def anti_spoofying_attack(name_list, data_dir, img_width, img_height, model, epsilons, path_save):
    idx = 0
    not_attack = 0
    attack_success = 0
    attack_fail = 0
    ...
    for img_name in name_list:
        ...
        if label == 1:
            not_attack += 1
            plt.imsave(os.path.join(path_save, f"adversarial_example_no_attack_confidence_{str(confidence)}.png"), adv_x)
            print(str(idx))
        ...
        if optimize_confidence < 0.5:
            attack_fail += 1
            plt.imsave(os.path.join(path_save, f"adversarial_example_{img_name[:-4]}_attack_fail.png"), optimize_adv)
        else:
            attack_success += 1
            ...
            #     psnr_fail_first += 1
            plt.imsave(os.path.join(path_save, f"adversarial_example_{img_name[:-4]}_eps_{str(optimize_eps)}_confidence_{str(optimize_confidence)}{psnr_str}.png"), optimize_adv)
        print(str(idx))
        print("not_attack: ", not_attack)
        print("attack_success: ", attack_success)
        print("attack_fail: ", attack_fail)
    ...
    f.write(f"attack_success_rate :  {str(attack_success_rate)}\n")
    f.close()
...
if __name__ == "__main__":
    ...
    name_list = []
    data_dir = "homemade_test/"
    path_save = "adversarial_example_ifgsm_range_homemade/"

    if os.path.exists(os.path.join(path_save, "attack success rate.txt")):
        os.remove(os.path.join(path_save, "attack success rate.txt"))
    origin_succes_rate(os.path.join(data_dir), img_width, img_height, model)
    # fgsm_epsilons = [0.02, 0.03]
    ifgsm_epsilons = [i * 0.01 for i in range(1, 10)]
    # ifgsm_epsilons = [0.05, 0.06]
    img_name_list = os.listdir(data_dir + '/')
    anti_spoofying_attack(img_name_list, data_dir, img_width, img_height, model, ifgsm_epsilons, path_save)
