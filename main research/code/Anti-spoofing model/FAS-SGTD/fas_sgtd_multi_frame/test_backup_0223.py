...
from generate_data_test_without_blank_depth_backup_0223 import input_fn_maker
from generate_network import generate_network as model_fn

os.environ["CUDA_VISIBLE_DEVICES"] = "0"
flags=FLAGS_backup_0223.flags
...
flags=FLAGS_backup_0223.flags # setting paras
...
def officialEvalSub(txt_name, data_list, mode, path_model_now):
    ...
    for feature in features:
        ...
        if out == labels[0] and out == 1:
            T_cnt += 1        
        elif out == labels[0] and out == 0:
            F_cnt += 1
    ...
    T_cnt_err = T_total - T_cnt
    F_cnt_err = F_total - F_cnt
    
    APCER = T_cnt_err / T_total
    BPCER = F_cnt_err / F_total
    ACER = (APCER + BPCER) / 2
