...
# tf1
import util_tf1.util_network as util_network
from util_tf1.util_network import *
import util_tf1.BasicConvLSTMCell as BasicConvLSTMCell
# tf2
...
def generate_network(features=[], mode=tf.estimator.ModeKeys.TRAIN):
    ...
    #                     'model_github', 'model.ckpt-19501'), 
    #                     {v.name.split(':')[0]: v for v in variables_to_restore})
    # tf.train.init_from_checkpoint(os.path.join( \
    #                     'model_save', 'model.ckpt'), 
    #                     {v.name.split(':')[0]: v for v in variables_to_restore})
    tf.compat.v1.train.init_from_checkpoint(os.path.join( \
                        flags.path.single_frame_model, 'model.ckpt-18700'), 
                        {v.name.split(':')[0]: v for v in variables_to_restore})
