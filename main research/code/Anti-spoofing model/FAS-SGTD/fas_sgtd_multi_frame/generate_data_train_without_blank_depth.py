...
def read_data_decode(name_pure, path_image, path_scene, start_ind, end_ind, label, face_name_full):
    ...
    if dataset == 'MSU-MFSD':
        scene_name_full = os.path.join(path_scene, name_pure, str(start_ind) + suffix4) if name_padding_zero == False else os.path.join(path_scene, name_pure, name_pure + '_' + '%03d' % start_ind + suffix4)
    else:
...
class InputFnGenerator:
    def __init__(self, train_list):
        def find_path_scene(path_depthmap):
            ...
            # print('path_gen_scene', path_gen_scene)
