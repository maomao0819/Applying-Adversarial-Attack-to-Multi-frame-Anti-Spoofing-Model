...
import time
import sys
sys.path.append('./util_tf1/')

import util_tf1.util_test_OULU_Protocol_1 as util_OULU
# from generate_data_test import input_fn_maker
from generate_data_test_without_blank_depth_tf1 import input_fn_maker
from generate_network_tf1 import generate_network as model_fn

os.environ["CUDA_VISIBLE_DEVICES"] = "1"
flags=FLAGS_tf1.flags
dataset = flags.path.dataset
inverse = flags.paras.inverse
logit_type = flags.paras.logit_type
...
# test_data_list=[flags.path.dev_file]
...
def ourEval():
    ...
    for feature in features:
        ...
        if inverse:
            ...
            prob= logits[0]>logits[1]
            acc= int(prob==labels[1])
        acc_mean += float(acc)
...
def officialEvalSub(txt_name, data_list, mode, path_model_now):
    ...
    def name_encode(name_):
        if mode == 'dev':
            ...
            if dataset == 'OULU-NPU':
                ...
                return dataset + name_
        else:
            print('Error mode: requires dev or test')
            exit(1)        
    
    eval_input_fn = lambda: input_fn_maker(data_list, shuffle=False, 
                                batch_size = 1,
                                epoch=1)
    ...
    for feature in features:
        ...
        if logit_type == 'paper':
            ...
            # 0.9590443686006825
                ...
                logits[1] = depth_ratio * depth_mean + cla_ratio * logits[1]
                logits[0] = 1.0 - depth_mean
    ...
    acer = (apcer + bpcer) / 2
    print('acc_mean:', acc_mean/float(fea_ind))
    print('apcer =', apcer, 'bpcer =', bpcer, 'acer =', acer)
    
    model_path = flags.path.model[2:-2] + '.txt'
    if not os.path.exists('test_result'):
        os.makedirs('test_result')

    with open(os.path.join('test_result', model_path), 'w+') as f:
        f.write(f'{path_model_now} : acc_mean = {acc_mean/float(fea_ind)}, apcer = {apcer}, bpcer = {bpcer}, acer = {acer}')
    fid.close()
