...
# path_gen_save = "./model_single_frame_multipler_2_inverse"
path_gen_save = "./model_multipler_2_inverse_19501"
...
if flags.path.dataset == "OULU-NPU":
    flags.path.train_file = [
        os.path.join(dataset_path, "mtcnn", "Train_features"),
        os.path.join(dataset_path, "Train_Depth_all"),
    ]
    flags.path.dev_file = [
        os.path.join(dataset_path, "mtcnn", "Dev_features"),
        os.path.join(dataset_path, "Dev_Depth_all"),
    ]
    flags.path.test_file = [
        os.path.join(dataset_path, "mtcnn", "Test_features"),
        os.path.join(dataset_path, "Test_Depth_all"),
    ]
elif flags.path.dataset == "CASIA-FASD":
    flags.path.train_file = [
        os.path.join(dataset_path, "Train_frame_mtcnn"),
        os.path.join(dataset_path, "Train_Depth_all"),
    ]
    flags.path.dev_file = "NoFile"
    flags.path.test_file = [
        os.path.join(dataset_path, "Test_frame_mtcnn"),
        os.path.join(dataset_path, "Test_Depth_all"),
    ]
    ...
    flags.path.dev_file = [
        os.path.join(dataset_path, "mtcnn", "Devel_frames_choose"),
        os.path.join(dataset_path, "Devel_Depth_frames_choose"),
    ]
    flags.path.test_file = [
        os.path.join(dataset_path, "mtcnn", "Test_features"),
        os.path.join(dataset_path, "Test_Depth_all"),
...
flags.dataset = edict()
flags.dataset.protocal = "ijcb_protocal_2"  #'ijcb_protocal_1'
if flags.path.dataset == "OULU-NPU":
...
flags.paras.isRealAttackPair = (
    False  # (real, print1/print2/replay1/replay2) or (real, print1, print2, replay1, replay2)
)
...
flags.paras.padding_info = {
    "images": [256, 256, 3 * flags.paras.len_seq],
...
#                             3: >= 0.5
#                             4: > 0.5
flags.paras.filter_type = 1
# flags.paras.filter_type:    1: gaussian_filter
#                             2: uniform_filter
flags.paras.filter_dimension = 3
flags.paras.filter_sigma = 1.2
flags.paras.filter_mode = 1
# flags.paras.filter_mode:    1: noise
#                             2: all image
#                             3: noise + all image
flags.paras.use_smooth_filter = 1
flags.paras.filter_time = 2
if flags.path.dataset == "CASIA-FASD" or flags.path.dataset == "replayattack":
