...
from util.util_dataset_spoofing import *
...
def read_data_decode(name_pure, path_image, path_scene, start_ind, end_ind, label, face_name_full):
    ...
    if label_inverse:
        ...
        if not label == 1:
            vertices_map_cat = np.zeros(vertices_map_cat.shape, dtype=np.float32)      
    folder_name = path_scene.split('/')[-1]
    #print(label, image_face_cat.shape, vertices_map_cat.shape, mask_cat.shape)
    ALLDATA=[image_face_cat, vertices_map_cat, mask_cat, folder_name]
...
def parser_fun(name_pure, path_image, path_scene, start_ind, end_ind, label, face_name_full):
    ...
    features['images']=tf.compat.v1.reshape(ALLDATA[0], padding_info['images']) / 255.0
    features['maps']=tf.compat.v1.reshape(ALLDATA[1], padding_info['maps'])
    features['masks']=tf.compat.v1.reshape(ALLDATA[2], padding_info['masks'])
    features['labels']=tf.compat.v1.reshape(label, padding_info['labels'])    
    features['names']=tf.compat.v1.reshape(tf.compat.v1.cast(name_pure, tf.compat.v1.string), [1])
    features['folder']=tf.compat.v1.reshape(tf.compat.v1.cast(ALLDATA[3], tf.compat.v1.string), [1])
    return features
...
def input_fn_maker(train_list, shuffle=True, batch_size=None, epoch=1, padding_info=None):
    def input_fn():
        ...
        ds=Dataset.from_generator(input_fn_handle, \
                     (tf.compat.v1.string, tf.compat.v1.string, tf.compat.v1.string, tf.compat.v1.int32, tf.compat.v1.int32, tf.compat.v1.int32, tf.compat.v1.string)
                     )
