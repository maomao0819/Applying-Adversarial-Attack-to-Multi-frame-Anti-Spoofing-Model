...
def image_augment(images):
    ...
    images = (images * float(255)).astype(np.uint8)
    # print("images2 ", images)
    # images_augment = (seq(images=images) / float(255)).astype(np.float32)
    images_augment = seq(images=images).astype(np.float32)
    # print("images3 ", images_augment)
    images_augment /= float(255)
    # print("images4 ", images_augment)
    if np.shape(images_augment)[-1] == 3:
        images_augment = np.moveaxis(images_augment, -1, 1)
    # print("images6 ", images_augment)
    return tf.convert_to_tensor(images_augment)
...
def perturbations(image, index, model):
    ...
    # print("ori ", image)
    img_aug = image_augment(image)
    # print("aug ", img_aug)
    return create_adversarial_pattern(img_aug, label, model)
