...
def boundary_attack():
    ...
    path_save = "adversarial_example_boundary_attack_for_testing_data_test_100_124"
            ...
            if int(dname_list[2]) > 100:
                ...
                for img_name in os.listdir(os.path.join(path_read_attack_image, dname)):
                    ...
                    for image_exist in os.listdir(os.path.join(path_save, dname)):
                        if img_name == image_exist.split('_')[-1]:
                            is_exists = True
                            break
                    if is_exists:
                        continue
                    ...
                    while True:
                        ...
                        if n_steps >= step_limit:
                            ...
                            if (model.predict(moveaxis_change_channel_order(adversarial_sample)) > 0.5)[0][0].astype("int32") == attack_class:
                                ...
                                plt.imsave(os.path.join(path_save, dname, f'n_steps_{str(n_steps)}_real_{random_image.replace(".png", "")}_attack_{img_name}'), adversarial_sample[0])
                                break
